...
semogaaaa Zazaaaa di tahun iniii sehat selaluuu, dilancarkan urusannyaa, dilimpahkan rezekinyaa, dikelilingi manusia-manusia baiikk, diperbanyak rasa syukurnya, diperbesar rasa sabarnyaaa, dibanggakan oleh banyaakk oraanngg, disayangi oleh banyaakk oraanngg, dikuatkan atas segala ujiannyaa, dijauhkan dari segala hal buruk, dijauhkan dari pikiran negatif, dijauhkan dari rasa sedih, kalaupun suatu hari nanti Zazaaaa harus berhadapan dengan hal itu, Yama percayaa pastiii Zazaaaa akan selalu bisa ngendaliin dan ngadepinnya.
...
function typeParagraph() {
    ...
    // Tampilkan foto Zaza saat mulai mengetik paragraf pertama
    ...
    // Tampilkan foto bunga hyacinth saat mulai mengetik paragraf kedua
    ...
    if (charIndex < text.length) {
        ...
        charIndex++;
        setTimeout(typeParagraph, char === "\n" ? 500 : 85);
    } else {
        // Selesai mengetik paragraf pertama
        if (pIndex === 0) {
            isTypingComplete = true;
            giftNextBtn.style.display = "block";
        }
        // Selesai mengetik paragraf kedua
        else if (pIndex === 1) {
            isTypingComplete = true;
            lastGiftBtn.style.display = "block";
        }
    }
}

// Button untuk lanjut dari paragraf 1 ke paragraf 2
giftNextBtn.onclick = () => {
...
// Button untuk buka video YouTube
lastGiftBtn.onclick = () => {
    if (isTypingComplete && pIndex === 1) {
        lastGiftBtn.style.display = "none";
        
        // Fade out bunga hyacinth dan teks
        flower.style.opacity = "0";
        quote.style.opacity = "0";
        typing.style.opacity = "0";

        setTimeout(() => {
            // Sembunyikan semua konten paragraf
            typing.style.display = "none";
            flower.style.display = "none";
            quote.style.display = "none";

            // Ubah layout ke video mode (full width)
            const giftWrapper = document.querySelector('.gift-wrapper');
            const textBox = document.querySelector('.text-box');
            const imageBox = document.querySelector('.image-box');
            
            giftWrapper.classList.add('video-mode');
            textBox.classList.add('hidden');
            imageBox.classList.add('video-mode');
